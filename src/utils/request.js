 ...
 * @type {*|string}
 */
let apiBaseUrl = process.env.NODE_ENV ==='development'? "/api":''
  ...
  if (error.response) {
    ...
    switch (error.response.status) {
      ...
      case 500:
        ...
        break
      case 404:
        this.$message.error('系统提示:很抱歉，资源未找到!');
        break
      case 504:
        this.$message.error('系统提示:网络超时!');
        break
      case 401:
        this.$message.error('系统提示:很抱歉，登录已过期，请重新登录!');
        if (token) {
          store.dispatch('Logout').then(() => {
            setTimeout(() => {
              window.location.reload()
