  ...
  <div class="box">
    ...
    <div v-show="isShowGold" class="modal" @click.stop="test()">
      <div v-if="buySuccess == '1'" class="buy-gold" @click.stop>
        ...
        <p class="loding-tex" v-if="loadingo">Blindbox is on the way</p>

        <div class="modal-bt" @click.stop="buyFunc" v-if="!loadingo">
          Buy Now
        </div>
      ...
      <div v-else-if="buySuccess == '4'" class="buy-gold3" @click.stop>
        <div class="success-title2">
          ...
          <img
            :src="require('../../../assets/hero/' + appear + '_y.png')"
            class="Nftimg"
          />
        ...
        <div class="modal-btt" @click="NTFbutcli()">Check It Out</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async buyFunc() {
      ...
      //可以获取当前购买盲盒的tokenid
      let resu = await sdk.HeroLogic.decodeEventBuyHero(buyboxx);
      ...
      //后台添加购买盲盒接口
      post("client/api/blindBox/buyHeroBlindBox", {
        type: this.buyboxher,
        price: this.msgG,
        heroId: this.tokenid,
    ...
    async openBox() {
      ...
      if (this.buyboxher == 1) {
        this.imgSrc2 = blue;
      } else if (this.buyboxher == 2) {
        this.imgSrc2 = pink;
      } else if (this.buyboxher == 3) {
        this.imgSrc2 = yellow;
      }
      ...
      this.appear = rrs.appearance; //图片地址拼接
      this.nickid = rrs.nick; //获取开出盲盒的nick值进行查询NFT名字
      this.level = rrs.quality_hero; //英雄稀有度
      this.job = rrs.job;
      this.owner = rrs.owner;
      this.skill1 = rrs.skill1;
      this.skill2 = rrs.skill2;
      this.tokenid = rrs.tokenid;
      ...
      get("client/api/blindBox/getHeroName", {
        ...
        this.NFTstr.IDs = this.nickname;
      });

      //将盲盒数据存到后台
      post("client/api/wallet/addNft", {
        address: this.owner,
        number: this.tokenid,
        type: "0",
        ...
        this.appear = res.data.msg;
      });
      this.NFTstr.tokenIDs = this.tokenid;
      this.NFTstr.Gra = this.level;
      this.NFTstr.imgS = this.appear;
